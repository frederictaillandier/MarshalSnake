...
public static class Native
{
    ...
    public static void Invoke<T>(IntPtr library, Action<int, string> onCreateObject, Action<int, Vector3> onPositionChange, Action<string> log)
    {
        IntPtr funcPtr = GetProcAddress(library, typeof(T).Name);
        if (funcPtr == IntPtr.Zero)
        {
        ...
        var func = Marshal.GetDelegateForFunctionPointer(funcPtr, typeof(T));
        func.DynamicInvoke(onCreateObject, onPositionChange, log);
    }
